<template>
  <div class="m-categroy">
    <dl class="categroy">
      <dt>按拼音首字母选择：</dt>
      <dd v-for="(item,index) in letterArray" :key="index">
        <a :href="'#city_'+ item ">{{ item }}</a>
      </dd>
    </dl>
    <dl class="m-categroy-section" v-for="item in city" :key="item.initial" :id="'city_'+item.initial">
    <dt>{{ item.initial }}</dt>
    <dd>
        <span v-for="(v,i) in item.list" :key="i">{{ v.name }}</span>
    </dd>
    </dl>
  </div>
</template>
